<!DOCTYPE html>
<html lang="en">
<html>
<head>
  <title>Students</title>
  <link href="./dist/css/bootstrap.css" rel="stylesheet">
  <link href='http://fonts.googleapis.com/css?family=Cabin+Sketch' rel='stylesheet' type='text/css'>
  <style type="text/css">
    body{
background: url("./images/wood1.png");
background-repeat:repeat;
}
    
    img {
    position: relative;
    float:left;
    }
  </style>
</head>

<body>

 
<div class="navbar navbar-inverse navbar-fixed-top" role="navigation">
    <div class="container">
    
      <div class="navbar-header" style="height: 50px;">
    
          
            <a class="navbar-brand" href="home_v9.html"><h1 style="font-family:'Cabin Sketch' cursive; margin-top: -9px;">LearnEmp<h1></a>
          </div>
          <div class="collapse navbar-collapse">
            <ul class="nav navbar-nav">
                <li><a href="listofstudents_v8.html">Sponsor</a></li>
                <li><a href="#">Sign Up</a></li>
            </ul>

        <div class="navbar-collapse collapse">
              <form class="navbar-form navbar-right" role="form">
                <div class="form-group">
                  <input type="text" placeholder="Email" class="form-control">
                </div>

                <div class="form-group">
                  <input type="password" placeholder="Password" class="form-control">
                </div>

                <button type="submit" class="btn btn-success" style="font-family: verdana;">Sign in</button> 
                &nbsp 

              </form>
              
              </div><!--/.navbar-collapse -->
        </div>
      </div>
      </div> 
  <!--</div> -->

    <br>
    <br>
    <br>
    <br>

  <div class="row">
    <div class="col-md-12" width="1000px;">
      <div class="well" width="px">  
        <img src="./images/donor.jpg" width="200px" height="200px" style="margin:10px;    margin-right:30px;"/> 
        <button type="submit" class="btn btn-success" style="font-family: verdana; float:right; margin-right:50px; width:150px; margin-top:20px;"><h4>Edit Profile</h4></button>
        
        <h2><strong>John Carter</strong></h2>

        <p>Los Angeles, CA . Joined since Feb 2014 </p>
        <p style="font-size:18px"> Born and raised in LA. Currently working selling electric vehicles and project planning for rad new projects taking place in LA. English Lit. graduate that loves to write and live life to the fullest!</>
        <br>
        <br>
        <p style="font-size:22px">Donated to 9 Students</p>
      </div>
  
    </div>
  </div>

  <div class="container" align="center">
    <h1 style="text-align:center; font-family:Cabin Sketch">John's Donated Student List</h1>
    <hr>
    
    <div class="row">
      <div class="col-lg-4" style="width:350px; margin-right: 60px;">
        <img src="./images/student03.jpg" alt="donate"align="center" style="height:200px; width:300px" ></img> 

        <br><h3>Priyanka</h3>
      </div>

      <div class="col-lg-4" style="width:350px; margin-right: 60px;">
          <a href="studentprofile_v5.html">
          <img src="./images/student.jpg" alt="donate" style="height:200px; width:300px"></img> </a>
          <br>
          <h3>Vaibhav Mule</h3>
      </div>

      <div class="col-lg-4" style="width:350px;">
          <img src="./images/student01.jpg" alt="donate"  style="height:200px; width:300px" ></img>
          <h3>Maria</h3>
      </div>
    </div>
<hr>
    <div class="row">
      
      <div class="col-lg-4" style="width:350px; margin-right: 60px;">
        <img src="./images/student03.jpg" alt="donate"align="center" style="height:200px; width:300px" ></img> 
        <br>
        <h3>Priyanka</h3>
      </div>

      <div class="col-lg-4" style="width:350px; margin-right: 60px;">
        <a href="studentprofile_v5.html">
        <img src="./images/student.jpg" alt="donate" style="height:200px; width:300px"></img>
        </a>

        <br>
        <h3>Vaibhav Mule</h3>

      </div>

      <div class="col-lg-4" style="width:350px;">
        <img src="./images/student01.jpg" alt="donate" style="height:200px; width:300px" ></img>

        <h3>Maria</h3>
      </div>
    </div>
<hr>
    <div class="row">
      
      <div class="col-lg-4" style="width:350px; margin-right: 60px;">
        <img src="./images/student03.jpg" alt="donate"align="center" style="height:200px; width:300px" ></img> 
        <br>
        <h3>Priyanka</h3>
      </div>

      <div class="col-lg-4" style="width:350px; margin-right: 60px;">
        <a href="studentprofile_v5.html">
        <img src="./images/student.jpg" alt="donate" style="height:200px; width:300px"></img> 
        </a>
        <br>
        <h3>Vaibhav Mule</h3>
      </div>

      <div class="col-lg-4" style="width:350px;">
        <img src="./images/student01.jpg" alt="donate"  style="height:200px; width:300px" ></img>
        <h3>Maria</h3>
      </div>
    </div>
  </div>

<br>
<br>
  <div class="row">
    <footer class="footer" style="text-align:center" class="row">
      <nav>
        <ul>
          <a href="home_v3.html">Home </a> |
          <a href= "#about">About Us</a> |
          <a href= "#hiw">How It Works</a> |
          <a href="#faq">FAQ</a> |
          <a href="#contact">Contact</a>  
        </ul>
      </nav> 
    </footer>
  </div>
</body>

</html> 